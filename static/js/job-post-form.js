(function () {
  ...
  function prefillFromJob(f, j) {
    ...
    if (qs("jpf_company_name")) qs("jpf_company_name").value = j.company_name || "";
    ...
    if (qs("jpf_about")) qs("jpf_about").value = j.about || "";
    var resp = j.responsibilities;
    if (Array.isArray(resp) && resp.length && qs("jpf_responsibilities")) {
      qs("jpf_responsibilities").value = resp.join("\n");
    } else if (qs("jpf_about") && !qs("jpf_about").value && j.description) {
      qs("jpf_about").value = j.description;
    }
    var rs = j.required_skills;
    if (Array.isArray(rs) && qs("jpf_required_skills")) qs("jpf_required_skills").value = rs.join(", ");
    ...
    var branches = j.branches_allowed || j.eligible_branches || [];
    ...
    if (qs("jpf_joining_date") && j.joining_date) qs("jpf_joining_date").value = j.joining_date.slice(0, 10);
    var rounds = j.rounds || [];
  ...
  document.addEventListener("DOMContentLoaded", function () {
    var root = document.getElementById("jobPostFormApp");
    ...
    if (editId) {
      var h = qs("jpf_page_title");
      var sub = qs("jpf_page_subtitle");
      var bc = qs("jpf_breadcrumb_current");
      if (h) h.textContent = "Edit job posting";
      if (sub) sub.textContent = "Update role details, eligibility, and attachments.";
      if (bc) bc.textContent = "Edit";
      if (submitBtn) submitBtn.textContent = "Save changes";
      fetch("/api/jobs/" + encodeURIComponent(editId), { credentials: "same-origin" })
        .then(function (r) {
        ...
        .then(function (data) {
          prefillFromJob(formEl, data.job);
        })
        .catch(function (e) {
    ...
    formEl.addEventListener("submit", function (e) {
      ...
      var method = "POST";
      if (editId) {
        url = "/api/jobs/" + encodeURIComponent(editId);
        method = "PUT";
      }
      ...
      fetch(url, { method: method, body: fd, credentials: "same-origin" })
        ...
        .then(function (res) {
          ...
          var mode = root.getAttribute("data-redirect-mode") || "list";
          if (mode === "detail" && res.data.id) {
            window.location.href = "/jobs/" + encodeURIComponent(res.data.id);
          } else if (mode === "detail" && editId) {
            window.location.href = "/jobs/" + encodeURIComponent(editId);
          } else {
            var u = root.getAttribute("data-success-url");
            window.location.href = u || "/jobs";
          }
        })
        .catch(function (er) {
